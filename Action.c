...
//add
static Htop_Reaction actionLimit(State* st) {
   Panel* limitPanel = (Panel*) LimitPanel_new();
   ListItem* sgn = (ListItem*) Action_pickFromVector(st, limitPanel, 50, true);
   if (sgn) {
      if (sgn->key != 0) {
         MainPanel_foreachProcess((MainPanel*)st->panel, (MainPanel_ForeachProcessFn) Process_sendLimit, (Arg){ .i = sgn->key }, NULL);

         napms(500);
      }
   }
   Panel_delete((Object*)limitPanel);
   return HTOP_REFRESH | HTOP_REDRAW_BAR | HTOP_UPDATE_PANELHDR;
}
//add
static Htop_Reaction actionHeaderhide(State* st) {
   ScreenManager* scr = ScreenManager_new(0, 0, 0, 0, HORIZONTAL, 0, st->settings, true);
   ScreenManager_add(scr, st->panel, 0);
   return HTOP_REFRESH | HTOP_REDRAW_BAR | HTOP_UPDATE_PANELHDR;
}
...
void Action_setBindings(Htop_Action* keys) {
   ...
   keys[KEY_F(11)] = actionLimit; // add
keys[KEY_F(12)] = actionHeaderhide; // add
}
